...
while getopts "D:p:B:s:m:H:n:G:R:P:l:T:c:i:t:o:" opt; do
  case $opt in
...
dbslmm=/public/home/biostat07/Five_people/code//DBSLMM/scr/dbslmm
...
for nchr in `seq 1 22`
do
cat ${summary_file_prefix}.assoc.txt | awk -v chr=${nchr} '$1 == chr' > ${summary_file_prefix}_chr${nchr}.assoc.txt
done
	...
	do
		...
		do
			...
			${plink}  --silent --bfile ${val_geno} --score ${outpath}${summchr_prefix}_h2f${h2f}.dbslmm.txt 1 2 4 sum\
					  --out ${outpath}${summchr_prefix}_h2f${h2f} --memory 6144
...
echo heritability is smaller than 0
fi
