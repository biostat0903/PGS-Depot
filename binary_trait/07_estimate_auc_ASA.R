...
#Merge two files
last_binary<-cbind(binary_ASA,ASA_list)
...
get_resid <- function(dat,
  ...
  write.table(extract_qqnorm[,2],
              file = paste0('/public/home/biostat07/ASA_binary/liner_model/rstandard/',phenoname[xvar-5]), 
              row.names = F, col.names = F, quote = F)
  write.table(pheno_eid[,2],
              file = paste0('/public/home/biostat07/ASA_binary/liner_model/pheno_value_eid/',phenoname[xvar-5]), 
              row.names = F, col.names = F, quote = F)
}
...
get_cov <- function(cov){
  ...
  write.table(cov.yhat,
              file=paste0('/public/home/biostat07/ASA_binary/cov_yhat/',temp_name[i],'.txt'),
...
get_auc_fun<-function(model,
  ...
  pred_tot <- vector()
  for(chr in 1: 22){
    pred_chr_str <- paste0('/public/home/biostat07/ASA_binary/output_summary/',file_name,'.effect/',
  ...
  if(length(pred_tot)==0){
    ...
    pheno_value<-fread2(paste0('/public/home/biostat07/ASA_binary/cov_yhat/',file_name,'.txt'),header=F)
    predict_value<-pheno_tot+pheno_value[,2]
    pheno_value[,2]<-predict_value
    pheno_value[,3]<-fread2(paste0('/public/home/biostat07/ASA_binary/liner_model/pheno_value_eid/',file_name,'.txt'),header=F)[,1]
    ...
    write.table(auc_sum,
                file=paste0('/public/home/biostat07/ASA_binary/output_summary/',file_name,
