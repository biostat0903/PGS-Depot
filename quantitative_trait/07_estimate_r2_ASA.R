...
library(bigreadr)
library(stringr)
load('/public/home/biostat07/ASA/temp/ASA_frame.RData')
load('/public/home/biostat07/ASA/temp/nosex_sort.RData')
load('/public/home/biostat07/ASA/temp/last_nosex.RData')
load('/public/home/biostat07/ASA/temp/last_female.RData')
load('/public/home/biostat07/ASA/temp/last_male.RData')
load('/public/home/biostat07/ASA/temp/temp_name.RData')
load('/public/home/biostat07/ASA/temp/summary_adjust_name.RData')
...
#Screening the ASA population and sorting the covariate files of the ASA population by fixed eid
ASA_list<-total_list[which(total_list$eid %in% ASA_eid),]
rownames(ASA_list)<-paste0('S',ASA_list$eid)
ASA_list<-ASA_list[paste0('S',ASA_eid),]
#Merge two files
last_ASA<-cbind(ASA_frame,ASA_list)
last_ASA_only_males<-last_ASA[which(last_ASA$sex==1),]
last_ASA_only_females<-last_ASA[which(last_ASA$sex==0),]
last_low50_males<-last_ASA[which(last_ASA$sex==1 & last_ASA$age<=50),]
last_over50_males<-last_ASA[which(last_ASA$sex==1 & last_ASA$age>50),]
last_low50_females<-last_ASA[which(last_ASA$sex==0 & last_ASA$age<=50),]
last_over50_females<-last_ASA[which(last_ASA$sex==0 & last_ASA$age>50),]
last_ASA_active<-last_ASA[which(last_ASA$activity>=5),]
last_ASA_inactive<-last_ASA[which(last_ASA$activity<5),]
write.table(last_ASA[,c(1,128)],
            file = "/public/home/biostat07/ASA/plink/plink_nosex.txt", 
            row.names = F, col.names = F, quote = F)
write.table(last_ASA_only_females[,c(1,128)],
            file = "/public/home/biostat07/ASA/plink/plink_female.txt", 
            row.names = F, col.names = F, quote = F)
write.table(last_ASA_only_males[,c(1,128)],
            file = "/public/home/biostat07/ASA/plink/plink_male.txt", 
            row.names = F, col.names = F, quote = F)
write.table(last_low50_females[,c(1,128)],
            file = "/public/home/biostat07/ASA/plink/plink_female_low50.txt", 
            row.names = F, col.names = F, quote = F)
write.table(last_low50_males[,c(1,128)],
            file = "/public/home/biostat07/ASA/plink/plink_male_low50.txt", 
            row.names = F, col.names = F, quote = F)
write.table(last_over50_females[,c(1,128)],
            file = "/public/home/biostat07/ASA/plink/plink_female_over50.txt", 
            row.names = F, col.names = F, quote = F)
write.table(last_over50_males[,c(1,128)],
            file = "/public/home/biostat07/ASA/plink/plink_male_over50.txt", 
            row.names = F, col.names = F, quote = F)
write.table(last_ASA_active[,c(1,128)],
            file = "/public/home/biostat07/ASA/plink/plink_active.txt", 
            row.names = F, col.names = F, quote = F)
write.table(last_ASA_inactive[,c(1,128)],
            file = "/public/home/biostat07/ASA/plink/plink_inactive.txt", 
            row.names = F, col.names = F, quote = F)
#preparation
...
get_resid <- function(dat,
  ...
  extract_qqnorm<-data.frame(last_ASA[,1])
  extract_qqnorm <- cbind(extract_qqnorm, extract_eid[,2])
  write.table(extract_qqnorm[,2],
              file = paste0('/public/home/biostat07/ASA/liner_model/rstandard/',phenoname[xvar-6]), 
              row.names = F, col.names = F, quote = F)
  write.table(pheno_eid,
              file = paste0('/public/home/biostat07/ASA/liner_model/pheno_value_eid/',phenoname[xvar-6]), 
              row.names = F, col.names = F, quote = F)
}
for (i in 1:length(var_names)+6) {
  if (str_split_fixed(var_names[i-6],'\\\(',n=2)[,2]=='age,nosex)'){
    get_resid(last_ASA,last_ASA,'age,sex',i)
  } else if (str_split_fixed(var_names[i-6],'\\\(',n=2)[,2]=='females)'){
    get_resid(last_ASA_only_females,ASA_female,'none',i)
  } else if (str_split_fixed(var_names[i-6],'\\\(',n=2)[,2]=='males)'){
    get_resid(last_ASA_only_males,ASA_male,'none',i)
  } else if (str_split_fixed(var_names[i-6],'\\\(',n=2)[,2]=='BMI)'){
    get_resid(last_ASA,last_ASA,'BMI',i)
    ...
    get_resid(last_ASA_active,last_ASA_active,'none',i)
  } else if (str_split_fixed(var_names[i-6],'\\\(',n=2)[,2]=='physically_inactive_indivdiuals)'){
    get_resid(last_ASA_inactive,last_ASA_inactive,'none',i)
  } else if (str_split_fixed(var_names[i-6],'\\\(',n=2)[,2]=='physical_activity)'){
    get_resid(last_ASA,last_ASA,'acttype',i)
  } else {
    get_resid(last_ASA,last_ASA,'none',i)
  }
}
#Function 2 :get r2
get_r2_fun<-function(model,
  ...
  pred_tot <- vector()
  for(chr in 1: 22){
    pred_chr_str <- paste0('/public/home/biostat07/ASA/output_summary/',file_name,'.effect/',
  ...
  if(length(pred_tot)==0){
    ...
    pheno_value<-fread2(paste0('/public/home/biostat07/ASA/liner_model/rstandard/',file_name,'.txt'),header=F)[,1]
    ...
    write.table(r2_sum,
                file=paste0('/public/home/biostat07/ASA/output_summary/',file_name,
